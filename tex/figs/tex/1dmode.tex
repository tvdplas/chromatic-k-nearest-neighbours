
\begin{figure}[H]
    \begin{center}
        \subfloat[1D-\{1,10,100\}-$k$-20-0-0]{
            \resizebox{8cm}{!}{
                \begin{tikzpicture}
                    \begin{axis}[
                            title={Mode query times for 1D-\{1,10,100\}-$k$-20-0-0},
                            xlabel={k},xmode=log,
                            ylabel={Mode query time (ms)},
                            legend pos=outer north east,
                            ymajorgrids=true,
                            grid style=dashed,
                            legend cell align={left}
                        ]
                        \addplot[
                            color=cyan,
                            mark=square,
                            dashed, thick,
                        ]
                        coordinates {
                                (10, 2.2)
                                (25, 2.3)
                                (50, 2.8)
                                (75, 3.3)
                                (100, 3.3)
                                (250, 5.3)
                                (500, 7.7)
                                (750, 12.8)
                            };
                        \addlegendentry{$n=1e3$ Naive}
                        \addplot[
                            color=cyan,
                            mark=o,
                        ]
                        coordinates {
                                (10, 2.0)
                                (25, 1.6)
                                (50, 1.2)
                                (75, 1.3)
                                (100, 1.0)
                                (250, 0.9)
                                (500, 0.8)
                                (750, 0.8)
                            };
                        \addlegendentry{$n=1e3$ Fast}

                        \addplot[
                            color=purple,
                            mark=square,
                            dashed, thick
                        ]
                        coordinates {
                                (10, 2.3)
                                (25, 2.4)
                                (50, 2.5)
                                (75, 2.8)
                                (100, 3.0)
                                (250, 5.3)
                                (500, 7.3)
                                (750, 10.0)
                                (1000, 12.9)
                                (1500, 19.0)
                                (2000, 29.2)
                            };
                        \addlegendentry{$n=1e4$ Naive}
                        \addplot[
                            color=purple,
                            mark=o,
                        ]
                        coordinates {
                                (10, 2.2)
                                (25, 2.2)
                                (50, 2.3)
                                (75, 2.5)
                                (100, 2.7)
                                (250, 2.8)
                                (500, 2.3)
                                (750, 2.3)
                                (1000, 2.2)
                                (1500, 2.4)
                                (2000, 2.7)
                            };
                        \addlegendentry{$n=1e4$ Fast}

                        \addplot[
                            color=gray,
                            mark=square,
                            dashed, thick,
                        ]
                        coordinates {
                                (10, 3.9)
                                (25, 2.9)
                                (50, 3.0)
                                (75, 3.2)
                                (100, 3.7)
                                (250, 6.5)
                                (500, 11.5)
                                (750, 12.7)
                                (1000, 16.4)
                                (1500, 20.2)
                                (2000, 28.7)
                            };
                        \addlegendentry{$n=1e5$ Naive}
                        \addplot[
                            color=gray,
                            mark=o,
                        ]
                        coordinates {
                                (10, 3.9)
                                (25, 3.0)
                                (50, 3.1)
                                (75, 3.3)
                                (100, 3.9)
                                (250, 7.8)
                                (500, 9.2)
                                (750, 8.6)
                                (1000, 8.0)
                                (1500, 7.3)
                                (2000, 8.1)
                            };
                        \addlegendentry{$n=1e5$ Fast}
                    \end{axis}
                \end{tikzpicture}
            }}
        \subfloat[1D-\{1, 10, 100\}-$k$-100-150-95 ]{
            \resizebox{8cm}{!}{
                \begin{tikzpicture}
                    \begin{axis}[
                            xlabel={k},xmode=log,
                            ylabel={Mode query time (ms)},
                            legend pos=outer north east,
                            ymajorgrids=true,
                            grid style=dashed,
                            legend cell align={left}
                        ]
                        \addplot[
                            color=cyan,
                            mark=square,
                            dashed, thick,
                        ]
                        coordinates {
                                (10, 3.9)
                                (25, 3.9)
                                (50, 4.6)
                                (75, 5.5)
                                (100, 5.8)
                                (250, 8.2)
                                (500, 11.0)
                                (750, 15.2)
                            };
                        \addlegendentry{$n=1e3$ Naive}
                        \addplot[
                            color=cyan,
                            mark=o,
                        ]
                        coordinates {
                                (10, 3.0)
                                (25, 2.3)
                                (50, 1.4)
                                (75, 1.4)
                                (100, 1.4)
                                (250, 1.1)
                                (500, 0.8)
                                (750, 0.7)
                            };
                        \addlegendentry{$n=1e3$ Fast}

                        \addplot[
                            color=purple,
                            mark=square,
                            dashed, thick
                        ]
                        coordinates {
                                (10, 2.4)
                                (25, 2.6)
                                (50, 2.8)
                                (75, 3.1)
                                (100, 3.4)
                                (250, 5.3)
                                (500, 7.7)
                                (750, 10.5)
                                (1000, 13.9)
                                (1500, 20.4)
                                (2000, 27.0)
                            };
                        \addlegendentry{$n=1e4$ Naive}
                        \addplot[
                            color=purple,
                            mark=o,
                        ]
                        coordinates {
                                (10, 2.3)
                                (25, 2.3)
                                (50, 2.4)
                                (75, 2.7)
                                (100, 3.4)
                                (250, 2.2)
                                (500, 1.9)
                                (750, 2.0)
                                (1000, 1.8)
                                (1500, 1.9)
                                (2000, 1.7)
                            };
                        \addlegendentry{$n=1e4$ Fast}

                        \addplot[
                            color=gray,
                            mark=square,
                            dashed, thick,
                        ]
                        coordinates {
                                (10, 3.5)
                                (25, 3.1)
                                (50, 3.3)
                                (75, 5.1)
                                (100, 4.8)
                                (250, 6.5)
                                (500, 7.9)
                                (750, 11.1)
                                (1000, 15.3)
                                (1500, 21.9)
                                (2000, 26.2)
                            };
                        \addlegendentry{$n=1e5$ Naive}
                        \addplot[
                            color=gray,
                            mark=o,
                        ]
                        coordinates {
                                (10, 3.4)
                                (25, 3.2)
                                (50, 3.3)
                                (75, 5.1)
                                (100, 5.2)
                                (250, 9.8)
                                (500, 10.4)
                                (750, 9.8)
                                (1000, 10.1)
                                (1500, 9.2)
                                (2000, 7.6)
                            };
                        \addlegendentry{$n=1e5$ Fast}
                    \end{axis}
                \end{tikzpicture}
            }}
        \caption{Naive and fast mode query times on 1D artificial instances}
        \label{fig:1dresmodeartificial}
    \end{center}
\end{figure}

\begin{figure}[H]
    \begin{center}
        \subfloat[FRAC uniform and clustered]{
            \resizebox{8cm}{!}{
                \begin{tikzpicture}
                    \begin{axis}[
                            title={Mode query times for FRAC},
                            xlabel={k},xmode=log,
                            ylabel={Mode query time (ms)},
                            legend pos=outer north east,
                            ymajorgrids=true,
                            grid style=dashed,
                            legend cell align={left}
                        ]
                        \addplot[
                            color=cyan,
                            mark=square,
                            dashed, thick,
                        ]
                        coordinates {
                                (1000, 14.3)
                                (2000, 26.1)
                                (3000, 37.0)
                                (4000, 48.3)
                                (5000, 59.7)
                                (6000, 75.4)
                                (7000, 80.3)
                                (8000, 98.3)
                                (9000, 112.5)
                            };
                        \addlegendentry{Uniform Naive}
                        \addplot[
                            color=cyan,
                            mark=o,
                        ]
                        coordinates {
                                (1000, 2.6)
                                (2000, 2.6)
                                (3000, 1.9)
                                (4000, 1.6)
                                (5000, 1.4)
                                (6000, 1.1)
                                (7000, 0.8)
                                (8000, 0.6)
                                (9000, 0.3)
                            };
                        \addlegendentry{Uniform Fast}

                        \addplot[
                            color=purple,
                            mark=square,
                            dashed, thick
                        ]
                        coordinates {
                                (1000, 14.5)
                                (2000, 28.1)
                                (3000, 39.1)
                                (4000, 48.1)
                                (5000, 59.1)
                                (6000, 74.1)
                                (7000, 82.3)
                                (8000, 96.8)
                                (9000, 110.1)
                            };
                        \addlegendentry{Clustered Naive}
                        \addplot[
                            color=purple,
                            mark=o,
                        ]
                        coordinates {
                                (1000, 1.8)
                                (2000, 1.7)
                                (3000, 1.4)
                                (4000, 1.1)
                                (5000, 0.9)
                                (6000, 0.8)
                                (7000, 0.6)
                                (8000, 0.4)
                                (9000, 0.2)
                            };
                        \addlegendentry{Clustered Fast}
                    \end{axis}
                \end{tikzpicture}
            }}
        \subfloat[1D-TMP-\{LAT,LON\}]{
            \resizebox{8cm}{!}{
                \begin{tikzpicture}
                    \begin{axis}[
                            xlabel={k},xmode=log,
                            ylabel={Mode query time (ms)},
                            legend pos=outer north east,
                            ymajorgrids=true,
                            grid style=dashed,
                            legend cell align={left}
                        ]
                        \addplot[
                            color=cyan,
                            mark=square,
                            dashed, thick,
                        ]
                        coordinates {
                                (10, 2.0)
                                (25, 2.3)
                                (50, 2.5)
                                (75, 2.7)
                                (100, 3.1)
                                (250, 4.7)
                                (500, 6.9)
                                (750, 9.4)
                                (1000, 12.9)
                                (1500, 18.0)
                                (2000, 22.9)
                            };
                        \addlegendentry{LAT Naive}
                        \addplot[
                            color=cyan,
                            mark=o,
                        ]
                        coordinates {
                                (10, 2.1)
                                (25, 2.1)
                                (50, 2.0)
                                (75, 2.3)
                                (100, 2.2)
                                (250, 1.8)
                                (500, 1.6)
                                (750, 1.6)
                                (1000, 1.5)
                                (1500, 1.6)
                                (2000, 1.4)
                            };
                        \addlegendentry{LAT Fast}

                        \addplot[
                            color=purple,
                            mark=square,
                            dashed, thick
                        ]
                        coordinates {
                                (10, 2.3)
                                (25, 2.3)
                                (50, 2.7)
                                (75, 2.8)
                                (100, 3.4)
                                (250, 4.7)
                                (500, 7.3)
                                (750, 10.8)
                                (1000, 12.9)
                                (1500, 18.6)
                                (2000, 22.8)
                            };
                        \addlegendentry{LON Naive}
                        \addplot[
                            color=purple,
                            mark=o,
                        ]
                        coordinates {
                                (10, 2.4)
                                (25, 2.1)
                                (50, 2.2)
                                (75, 2.3)
                                (100, 2.3)
                                (250, 1.9)
                                (500, 1.6)
                                (750, 1.8)
                                (1000, 1.6)
                                (1500, 1.7)
                                (2000, 1.4)
                            };
                        \addlegendentry{LON Fast}
                    \end{axis}
                \end{tikzpicture}
            }}
        \caption{Naive and fast mode query times on 1D FRAC and TMP instances}
        \label{fig:1dresmoderl}
    \end{center}
\end{figure}

% All data

% % A1-A6
% \hskip-2cm
% \resizebox{8cm}{!}{
%     \begin{tikzpicture}
%     \begin{axis}[
%         title={Mode query times for A1-A3},
%         xlabel={k},xmode=log,
%         ylabel={Mode query time (ms)},
%         legend pos=outer north east,
%         ymajorgrids=true,
%         grid style=dashed,
%     ]
%     \addplot[
%         color=cyan,
%         mark=square,
%         dashed, thick,
%         ]
%         coordinates {
%             (10, 2.2)
%             (25, 2.3)
%             (50, 2.8)
%             (75, 3.3)
%             (100, 3.3)
%             (250, 5.3)
%             (500, 7.7)
%             (750, 12.8)
%         };
%     \addlegendentry{A1 Naive}
%     \addplot[
%         color=cyan,
%         mark=o,
%         ]
%         coordinates {
%             (10, 2.0)
%             (25, 1.6)
%             (50, 1.2)
%             (75, 1.3)
%             (100, 1.0)
%             (250, 0.9)
%             (500, 0.8)
%             (750, 0.8)
%         };
%     \addlegendentry{A1 Fast}

%     \addplot[
%         color=purple,
%         mark=square,
%         dashed, thick
%         ]
%         coordinates {
%             (10, 2.3)
%             (25, 2.4)
%             (50, 2.5)
%             (75, 2.8)
%             (100, 3.0)
%             (250, 5.3)
%             (500, 7.3)
%             (750, 10.0)
%             (1000, 12.9)
%             (1500, 19.0)
%             (2000, 29.2)
%         };
%     \addlegendentry{A2 Naive}
%     \addplot[
%         color=purple,
%         mark=o,
%         ]
%         coordinates {
%             (10, 2.2)
%             (25, 2.2)
%             (50, 2.3)
%             (75, 2.5)
%             (100, 2.7)
%             (250, 2.8)
%             (500, 2.3)
%             (750, 2.3)
%             (1000, 2.2)
%             (1500, 2.4)
%             (2000, 2.7)
%         };
%     \addlegendentry{A2 Fast}

%     \addplot[
%         color=gray,
%         mark=square,
%         dashed, thick,
%         ]
%         coordinates {
%             (10, 3.9)
%             (25, 2.9)
%             (50, 3.0)
%             (75, 3.2)
%             (100, 3.7)
%             (250, 6.5)
%             (500, 11.5)
%             (750, 12.7)
%             (1000, 16.4)
%             (1500, 20.2)
% (2000, 28.7)
%         };
%     \addlegendentry{A3 Naive}
%     \addplot[
%         color=gray,
%         mark=o,
%         ]
%         coordinates {
%             (10, 3.9)
%             (25, 3.0)
%             (50, 3.1)
%             (75, 3.3)
%             (100, 3.9)
%             (250, 7.8)
%             (500, 9.2)
%             (750, 8.6)
%             (1000, 8.0)
%             (1500, 7.3) 
% (2000, 8.1) 
%         };
%     \addlegendentry{A3 Fast}
%     \end{axis}
%     \end{tikzpicture}
% }
% \resizebox{8cm}{!}{
%     \begin{tikzpicture}
%     \begin{axis}[
%         title={Mode query times for A4-A6},
%         xlabel={k},xmode=log,
%         ylabel={Mode query time (ms)},
%         legend pos=outer north east,
%         ymajorgrids=true,
%         grid style=dashed,
%     ]
%     \addplot[
%         color=cyan,
%         mark=square,
%         dashed, thick,
%         ]
%         coordinates {
%             (10, 2.6)
%             (25, 2.8)
%             (50, 3.1)
%             (75, 3.2)
%             (100, 3.8)
%             (250, 6.2)
%             (500, 9.9)
%             (750, 13.9)
%         };
%     \addlegendentry{A4 Naive}
%     \addplot[
%         color=cyan,
%         mark=o,
%         ]
%         coordinates {
%             (10, 2.4)
%             (25, 1.9)
%             (50, 1.5)
%             (75, 1.3)
%             (100, 1.3)
%             (250, 1.1)
%             (500, 1.0)
%             (750, 0.8)
%         };
%     \addlegendentry{A4 Fast}

%     \addplot[
%         color=purple,
%         mark=square,
%         dashed, thick
%         ]
%         coordinates {
%             (10, 2.5)
%             (25, 2.6)
%             (50, 2.9)
%             (75, 3.1)
%             (100, 3.4)
%             (250, 5.0)
%             (500, 8.3)
%             (750, 10.3)
%             (1000, 14.6)
%             (1500, 21.0)
% (2000, 29.3)
%         };
%     \addlegendentry{A5 Naive}
%     \addplot[
%         color=purple,
%         mark=o,
%         ]
%         coordinates {
%             (10, 2.5)
%             (25, 2.5)
%             (50, 2.7)
%             (75, 3.2)
%             (100, 4.1)
%             (250, 3.2)
%             (500, 2.8)
%             (750, 2.6)
%             (1000, 2.5)
%             (1500, 2.9)
% (2000, 3.1)
%         };
%     \addlegendentry{A5 Fast}

%     \addplot[
%         color=gray,
%         mark=square,
%         dashed, thick,
%         ]
%         coordinates {
%             (10, 4.0)
%             (25, 3.5)
%             (50, 3.4)
%             (75, 3.9)
%             (100, 4.2)
%             (250, 7.9)
%             (500, 10.5)
%             (750, 13.4)
%             (1000, 15.2)
%             (1500, 21.3)
% (2000, 32.3)
%         };
%     \addlegendentry{A6 Naive}
%     \addplot[
%         color=gray,
%         mark=o,
%         ]
%         coordinates {
%             (10, 3.8)
%             (25, 3.7)
%             (50, 3.6)
%             (75, 4.4)
%             (100, 4.9)
%             (250, 11.0)
%             (500, 12.1)
%             (750, 11.2)
%             (1000, 9.5)
%             (1500, 9.7) 
% (2000, 10.7)
%         };
%     \addlegendentry{A6 Fast}
%     \end{axis}
%     \end{tikzpicture}
% }\\

% % A7-A12
% \hskip-2cm
% \resizebox{8cm}{!}{
%     \begin{tikzpicture}
%     \begin{axis}[
%         title={Mode query times for A7-A9},
%         xlabel={k},xmode=log,
%         ylabel={Mode query time (ms)},
%         legend pos=outer north east,
%         ymajorgrids=true,
%         grid style=dashed,
%     ]
%     \addplot[
%         color=cyan,
%         mark=square,
%         dashed, thick,
%         ]
%         coordinates {
%             (10, 2.5)
%             (25, 2.9)
%             (50, 3.7)
%             (75, 3.6)
%             (100, 4.0)
%             (250, 6.4)
%             (500, 9.4)
%             (750, 14.8)
%         };
%     \addlegendentry{A7 Naive}
%     \addplot[
%         color=cyan,
%         mark=o,
%         ]
%         coordinates {
%             (10, 2.1)
%             (25, 1.7)
%             (50, 1.5)
%             (75, 1.1)
%             (100, 1.0)
%             (250, 0.9)
%             (500, 0.8)
%             (750, 0.8)
%         };
%     \addlegendentry{A7 Fast}

%     \addplot[
%         color=purple,
%         mark=square,
%         dashed, thick
%         ]
%         coordinates {
%             (10, 2.0)
%             (25, 2.2)
%             (50, 2.5)
%             (75, 3.0)
%             (100, 3.2)
%             (250, 5.1)
%             (500, 7.6)
%             (750, 10.0)
%             (1000, 12.7)
%             (1500, 21.8)
% (2000, 27.2)
%         };
%     \addlegendentry{A8 Naive}
%     \addplot[
%         color=purple,
%         mark=o,
%         ]
%         coordinates {
%             (10, 1.9)
%             (25, 2.1)
%             (50, 2.2)
%             (75, 3.0)
%             (100, 3.6)
%             (250, 3.3)
%             (500, 2.3)
%             (750, 2.2)
%             (1000, 1.9)
%             (1500, 2.0) 
% (2000, 1.8) 
%         };
%     \addlegendentry{A8 Fast}

%     \addplot[
%         color=gray,
%         mark=square,
%         dashed, thick,
%         ]
%         coordinates {
%             (10, 3.0)
%             (25, 2.7)
%             (50, 3.0)
%             (75, 4.0)
%             (100, 4.6)
%             (250, 6.1)
%             (500, 7.5)
%             (750, 10.8)
%             (1000, 14.0)
%             (1500, 22.6)
% (2000, 26.0)
%         };
%     \addlegendentry{A9 Naive}
%     \addplot[
%         color=gray,
%         mark=o,
%         ]
%         coordinates {
%             (10, 3.2)
%             (25, 2.7)
%             (50, 3.0)
%             (75, 4.2)
%             (100, 4.8)
%             (250, 10.0)
%             (500, 11.1)
%             (750, 11.1)
%             (1000, 11.0)
%             (1500, 12.3)
% (2000, 9.9) 
%         };
%     \addlegendentry{A9 Fast}
%     \end{axis}
%     \end{tikzpicture}
% }
% \resizebox{8cm}{!}{
%     \begin{tikzpicture}
%     \begin{axis}[
%         title={Mode query times for A10-A12},
%         xlabel={k},xmode=log,
%         ylabel={Mode query time (ms)},
%         legend pos=outer north east,
%         ymajorgrids=true,
%         grid style=dashed,
%     ]
%     \addplot[
%         color=cyan,
%         mark=square,
%         dashed, thick,
%         ]
%         coordinates {
%             (10, 3.9)
%             (25, 3.9)
%             (50, 4.6)
%             (75, 5.5)
%             (100, 5.8)
%             (250, 8.2)
%             (500, 11.0)
%             (750, 15.2)
%         };
%     \addlegendentry{A10 Naive}
%     \addplot[
%         color=cyan,
%         mark=o,
%         ]
%         coordinates {
%             (10, 3.0)
%             (25, 2.3)
%             (50, 1.4)
%             (75, 1.4)
%             (100, 1.4)
%             (250, 1.1)
%             (500, 0.8)
%             (750, 0.7)
%         };
%     \addlegendentry{A10 Fast}

%     \addplot[
%         color=purple,
%         mark=square,
%         dashed, thick
%         ]
%         coordinates {
%             (10, 2.4)
%             (25, 2.6)
%             (50, 2.8)
%             (75, 3.1)
%             (100, 3.4)
%             (250, 5.3)
%             (500, 7.7)
%             (750, 10.5)
%             (1000, 13.9)
%             (1500, 20.4)
% (2000, 27.0)
%         };
%     \addlegendentry{A11 Naive}
%     \addplot[
%         color=purple,
%         mark=o,
%         ]
%         coordinates {
%            (10, 2.3)
%             (25, 2.3)
%             (50, 2.4)
%             (75, 2.7)
%             (100, 3.4)
%             (250, 2.2)
%             (500, 1.9)
%             (750, 2.0)
%             (1000, 1.8)
%             (1500, 1.9) 
% (2000, 1.7) 
%         };
%     \addlegendentry{A11 Fast}

%     \addplot[
%         color=gray,
%         mark=square,
%         dashed, thick,
%         ]
%         coordinates {
%             (10, 3.5)
%             (25, 3.1)
%             (50, 3.3)
%             (75, 5.1)
%             (100, 4.8)
%             (250, 6.5)
%             (500, 7.9)
%             (750, 11.1)
%             (1000, 15.3)
%             (1500, 21.9)
% (2000, 26.2)
%         };
%     \addlegendentry{A12 Naive}
%     \addplot[
%         color=gray,
%         mark=o,
%         ]
%         coordinates {
%             (10, 3.4)
%             (25, 3.2)
%             (50, 3.3)
%             (75, 5.1)
%             (100, 5.2)
%             (250, 9.8)
%             (500, 10.4)
%             (750, 9.8)
%             (1000, 10.1)
%             (1500, 9.2) 
% (2000, 7.6) 
%         };
%     \addlegendentry{A12 Fast}
%     \end{axis}
%     \end{tikzpicture}
% }

% % B1, B2, C1, C2
% \hskip-2cm
% \resizebox{8cm}{!}{
%     \begin{tikzpicture}
%     \begin{axis}[
%         title={Mode query times for B1 and B2},
%         xlabel={k},xmode=log,
%         ylabel={Mode query time (ms)},
%         legend pos=outer north east,
%         ymajorgrids=true,
%         grid style=dashed,
%     ]
%     \addplot[
%         color=cyan,
%         mark=square,
%         dashed, thick,
%         ]
%         coordinates {
%             (1000, 14.3)
%             (2000, 26.1)
%             (3000, 37.0)
%             (4000, 48.3)
%             (5000, 59.7)
%             (6000, 75.4)
%             (7000, 80.3)
%             (8000, 98.3)
%             (9000, 112.5)
%         };
%     \addlegendentry{B1 Naive}
%     \addplot[
%         color=cyan,
%         mark=o,
%         ]
%         coordinates {
%             (1000, 2.6)
%             (2000, 2.6)
%             (3000, 1.9)
%             (4000, 1.6)
%             (5000, 1.4)
%             (6000, 1.1)
%             (7000, 0.8)
%             (8000, 0.6)
%             (9000, 0.3)
%         };
%     \addlegendentry{B1 Fast}

%     \addplot[
%         color=purple,
%         mark=square,
%         dashed, thick
%         ]
%         coordinates {
%             (1000, 14.5)
%             (2000, 28.1)
%             (3000, 39.1)
%             (4000, 48.1)
%             (5000, 59.1)
%             (6000, 74.1)
%             (7000, 82.3)
%             (8000, 96.8)
%             (9000, 110.1)
%         };
%     \addlegendentry{B2 Naive}
%     \addplot[
%         color=purple,
%         mark=o,
%         ]
%         coordinates {
%             (1000, 1.8)
%             (2000, 1.7)
%             (3000, 1.4)
%             (4000, 1.1)
%             (5000, 0.9)
%             (6000, 0.8)
%             (7000, 0.6)
%             (8000, 0.4)
%             (9000, 0.2)
%         };
%     \addlegendentry{B2 Fast}
%     \end{axis}
%     \end{tikzpicture}
% }
% \resizebox{8cm}{!}{
%     \begin{tikzpicture}
%     \begin{axis}[
%         title={Mode query times for C1 and C2},
%         xlabel={k},xmode=log,
%         ylabel={Mode query time (ms)},
%         legend pos=outer north east,
%         ymajorgrids=true,
%         grid style=dashed,
%     ]
%     \addplot[
%         color=cyan,
%         mark=square,
%         dashed, thick,
%         ]
%         coordinates {
%             (10, 2.0)
%             (25, 2.3)
%             (50, 2.5)
%             (75, 2.7)
%             (100, 3.1)
%             (250, 4.7)
%             (500, 6.9)
%             (750, 9.4)
%             (1000, 12.9)
%             (1500, 18.0)
% (2000, 22.9)
%         };
%     \addlegendentry{C1 Naive}
%     \addplot[
%         color=cyan,
%         mark=o,
%         ]
%         coordinates {
%             (10, 2.1)
%             (25, 2.1)
%             (50, 2.0)
%             (75, 2.3)
%             (100, 2.2)
%             (250, 1.8)
%             (500, 1.6)
%             (750, 1.6)
%             (1000, 1.5)
%             (1500, 1.6)
% (2000, 1.4)
%         };
%     \addlegendentry{C1 Fast}

%     \addplot[
%         color=purple,
%         mark=square,
%         dashed, thick
%         ]
%         coordinates {
%             (10, 2.3)
%             (25, 2.3)
%             (50, 2.7)
%             (75, 2.8)
%             (100, 3.4)
%             (250, 4.7)
%             (500, 7.3)
%             (750, 10.8)
%             (1000, 12.9)
%             (1500, 18.6)
% (2000, 22.8)
%         };
%     \addlegendentry{C2 Naive}
%     \addplot[
%         color=purple,
%         mark=o,
%         ]
%         coordinates {
%             (10, 2.4)
%             (25, 2.1)
%             (50, 2.2)
%             (75, 2.3)
%             (100, 2.3)
%             (250, 1.9)
%             (500, 1.6)
%             (750, 1.8)
%             (1000, 1.6)
%             (1500, 1.7)
% (2000, 1.4)
%         };
%     \addlegendentry{C2 Fast}
%     \end{axis}
%     \end{tikzpicture}
% }
