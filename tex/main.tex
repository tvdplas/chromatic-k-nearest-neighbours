...
\newcommand{\fbnote}[1]{{\color{blue}#1}}
...
\textit{Preprocessing}: Sort the pointset $P$, save this sorted list. \\\\
\textit{Query}: Binary search to find the index of the point closest to the query point $q$ within the pointset $P$. Using this index $q_i$, then find the $k$th nearest point by stepping either left or right from $q_i$ in the index range $[q_i-k, q_i+k]$. \fb{Return the distance between $q$ and the $k$th nearest point.}
...
\textit{Preprocessing}: Sort the points, then save
points in a node-valued binary search tree $T$ that contains size annotations.
\\\\ \textit{Query}: Based on the query point $q$, split $T$ into two trees
...
$\ell$, we now have two distinct cases. \fb{These are visualised in Figure \ref{fig:vanderHorst_treesplit}, which appeared as Figure 3 of van der Horst et al. their work.} \\ 
...
\textit{Preprocessing}: \fb{Sort the pointset $P$, save this sorted list.} \\\\
\textit{Query}: \fb{Based on $q$ and the provided radius, determine the indices into $P$ that fall within the radius around $q$ using a binary search for the lower and upper bound. Afterwards, count the frequency of each color within our index range using a dictionary. Return the color with the highest counted frequency.}
